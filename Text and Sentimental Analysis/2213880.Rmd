...
Jessica_Plot <- Jessicas_words %>%
  slice_max(n, n = 25) %>%
  mutate(word = reorder(word, n)) %>%
  ggplot(aes(n, word)) + ggplot2::geom_col()
```
Fig 2 shows that Gabby's most used word in the Talk is "gabby" and Jessica's most used word is "assault" which occurred 8 times and followed closely by college which occurred 7 times in the Ted talk.

```{r}
grid.arrange(Gabby_Plot, Jessica_Plot, ncol=2)
```



```{r fig.cap="3", fig.align='center'}
...
```
In the Figure, the sentiment with the highest positive and negative occurrences for Gabby Giffords well(14 times) and hard(8 times) respectively and for Jessica Ladd, like for positive and assualt for negative. They appeared 5 and 8 times respectively.
