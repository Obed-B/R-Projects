...
#Tables
```{r plot, fig.align='center', fig.cap="Table showing races of Officers"}
table(OFFICER_RACE)

```
The figure depicts the distribution of reported cases of force applied to arrest subjects in Dallas in 2016, broken down by the race of the officer involved. The data reveal that white officers reported the highest number of cases, with a total of 1470 incidents, whereas American Indians had the lowest number, with only 8 cases. This finding underscores the potential role of race in shaping patterns of police behavior and highlights the need for further research to investigate the underlying factors driving these disparities.

```{r plot, fig.align='center', fig.cap="Table showing gender of Officers"}
round(100*prop.table(table(OFFICER_GENDER)),2)
```
The figure presented provides evidence that the majority of officers are males, accounting for 89.93% of the sample. In contrast, females make up only 10.07% of the officers. This observation is notable as it suggests a significant gender disparity in the officer population being studied. 
```{r plot, fig.align='center', fig.cap="Table showing race of subjects arrest by force"}
...
```{r plot, fig.align='center', fig.cap="Table showing gender of subjects"}
...
```{r plot, fig.align='center', fig.cap="Barplot showing Division frequency of Incidents"}
...
```{r plot, fig.align='center', fig.cap="Incidents counts  by Officers years spent on force"}
his=
...
#Boxplot
```{r plot, fig.align='center', fig.cap="Box Plot"}
...
ggplotly(
  ggplot(data = data_sel, aes(x = INCIDENT_DATE)) +
    geom_density(fill = "#2b8cbe", alpha = 0.5) +
